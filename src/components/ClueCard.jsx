...
import { Eye, EyeOff, Trophy, Award, Star } from 'lucide-react';
    ...
    <div
      className={`clue-card p-5 rounded-2xl border transition-all duration-300 mb-3 ${
        !revealed && !disabled
          ...
          : revealed
          ? 'revealed bg-gradient-to-r from-blue-900/40 to-purple-900/40 border-blue-500'
    ...
    >
      <div className="flex items-start gap-4">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold flex-shrink-0 transition-all ${
          ...
          {revealed ? getClueIcon() || (index + 1) : index + 1}
        </div>
        <div className="flex-1">
          <div className="flex justify-between items-center">
            ...
            {revealed ? (
              ...
              <EyeOff className={`text-slate-500 w-5 h-5 ${isHovered && !disabled ? 'text-blue-400' : ''}`} />
            )}
          </div>
        </div>
      </div>
